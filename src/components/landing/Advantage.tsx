        ...
        <section className="flex justify-center">
            <ul className="w-full md:w-[80%] py-5">
                {advantages.map((item, index) => (
                    ...
                    >
                        <li className="p-5 grid grid-cols-1 lg:grid-cols-2 justify-center items-center z-10 relative">
                            {index % 2 === 0 ? (
                                <>
                                    <div className="md:px-5 order-2 lg:order-1">
                                        <h2 className="text-2xl text-primary-700 font-bold pb-4">{item.title}</h2>
                                        <p className="text-xl text-primary-700 pb-4">{item.description}</p>
                                    </div>
                                    <div className="flex justify-center order-1 lg:order-2">
                                        <Image
                                            src={item.image}
                                            alt={item.title}
                                            className="order-1 lg:order-2 w-40 h-40 sm:w-48 sm:h-48 md:w-60 md:h-60 lg:w-72 lg:h-72"
                                        />
                                    </div>
                                </>
                            ) : (
                                <>
                                    ...
                                    <div className="md:px-5 order-2 lg:order-2">
                                        <h2 className="text-2xl text-primary-700 font-bold pb-4">{item.title}</h2>
                                        <p className="text-xl text-primary-700 pb-4">{item.description}</p>
                                    </div>
                                </>
